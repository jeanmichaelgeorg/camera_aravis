...
//#define TUNING	// Allows tuning the gains for the timestamp controller.  Publishes output on topic /dt, and
// receives gains on params /kp, /ki, /kd
...
ArvGvStream *CreateStream(void) {
  ...
  if (pStream) {
    ...
    if (!ARV_IS_GV_STREAM(pStream)) ROS_WARN("Stream is not a GV_STREAM");

    if (bAutoBuffer)
      g_object_set(pStream, "socket-buffer", ARV_GV_STREAM_SOCKET_BUFFER_AUTO, "socket-buffer-size", 0, NULL);
    if (!bPacketResend)
      g_object_set(pStream, "packet-resend",
                   bPacketResend ? ARV_GV_STREAM_PACKET_RESEND_ALWAYS : ARV_GV_STREAM_PACKET_RESEND_NEVER, NULL);
    g_object_set(pStream, "packet-timeout", (unsigned)timeoutPacket * 1000, "frame-retention",
                 (unsigned)timeoutFrameRetention * 1000, NULL);
...
void RosReconfigure_callback(Config &config, uint32_t level) {
  ...
  if (config.frame_id == "") config.frame_id = "camera";
  ...
  config.AcquisitionFrameRate =
      CLIP(config.AcquisitionFrameRate, global.configMin.AcquisitionFrameRate, global.configMax.AcquisitionFrameRate);
  config.ExposureTimeAbs =
      CLIP(config.ExposureTimeAbs, global.configMin.ExposureTimeAbs, global.configMax.ExposureTimeAbs);
  ...
  if (changedExposureTimeAbs || changedGainAuto ||
      ((changedAcquisitionFrameRate || changedGain || changedFrameid || changedAcquisitionMode ||
  ...
  if (changedGain || changedExposureAuto ||
      ((changedAcquisitionFrameRate || changedExposureTimeAbs || changedFrameid || changedAcquisitionMode ||
        changedTriggerSource || changedSoftwarerate) &&
       config.GainAuto == "Once"))
    config.GainAuto = "Off";

  if (changedAcquisitionFrameRate) config.TriggerMode = "Off";
  ...
  if ((changedTriggerMode || changedTriggerSource || changedSoftwarerate) && config.TriggerMode == "On" &&
      config.TriggerSource == "Software") {
    if (global.isImplementedAcquisitionFrameRate) {
      // The software rate is limited by the camera's internal framerate.  Bump up the camera's internal framerate if
      // necessary.
  ...
  if (changedTriggerSource || changedSoftwarerate) {
    ...
    if (!strcmp(config.TriggerSource.c_str(), "Software")) {
      ...
      global.idSoftwareTriggerTimer =
          g_timeout_add((guint)ceil(1000.0 / config.softwaretriggerrate), SoftwareTrigger_callback, global.pCamera);
    }
  }
  if (changedFocusPos) {
...
#ifdef TUNING
      ROS_WARN("en=%16ld, ie=%16ld, de=%16ld, u=%16ld + %16ld + %16ld = %16ld", en, ie, de, kp1 * (en / kp2),
               ki1 * (ie / ki2), kd1 * (de / kd2), u);
      ROS_WARN("cn=%16lu, rn=%16lu, cn-cm=%8ld, rn-rm=%8ld, tn-tm=%8ld, tn-rn=%ld", cn, rn, cn - cm, rn - rm,
               (int64_t)tn - (int64_t)tm, tn - rn);
...
// Walk the DOM tree, i.e. the tree represented by the XML file in the camera, and that contains all the various
...
// Read ROS parameters from this node's namespace, and see if each parameter has a similarly named & typed feature in
// the camera.  Then set the camera feature to that value.  For example, if the parameter camnode/Gain is set to 123.0,
// then we'll write 123.0 to the Gain feature in the camera.
//
// Note that the datatype of the parameter *must* match the datatype of the camera feature, and this can be determined
// by looking at the camera's XML file.  Camera enum's are string parameters, camera bools are false/true parameters
// (not 0/1), integers are integers, doubles are doubles, etc.
//
  ...
  if (xmlrpcParams.getType() == XmlRpc::XmlRpcValue::TypeStruct) {
    for (iter = xmlrpcParams.begin(); iter != xmlrpcParams.end(); iter++) {
      ...
      if (pGcNode && arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(pGcNode), &error)) {
        //				unsigned long	typeValue = arv_gc_feature_node_get_value_type((ArvGcFeatureNode
        //*)pGcNode); 				ROS_INFO("%s cameratype=%lu, rosparamtype=%d", key.c_str(), typeValue,
        ...
        switch (iter->second.getType()) {
          case XmlRpc::XmlRpcValue::TypeBoolean:  // if ((iter->second.getType()==XmlRpc::XmlRpcValue::TypeBoolean))//
          ...
          case XmlRpc::XmlRpcValue::TypeDouble:  // if ((iter->second.getType()==XmlRpc::XmlRpcValue::TypeDouble))// &&
          ...
          case XmlRpc::XmlRpcValue::TypeString:  // if ((iter->second.getType()==XmlRpc::XmlRpcValue::TypeString))// &&
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < nDevices; i++) ROS_INFO("Device%d: %s", i, arv_get_device_id(i));
  ...
  if (nDevices > 0) {
    ...
    ROS_INFO("Opened: %s-%s", arv_device_get_string_feature_value(global.pDevice, "DeviceVendorName"),
    ...
    global.isImplementedAcquisitionMode =
        ARV_GC_FEATURE_NODE(pGcNode) ? arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(pGcNode), &error) : FALSE;

    pGcNode = arv_device_get_feature(global.pDevice, "GainRaw");
    global.isImplementedGain =
        ARV_GC_FEATURE_NODE(pGcNode) ? arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(pGcNode), &error) : FALSE;
    ...
    for (i = 0; i < 2; i++) {
      ...
      global.isImplementedAcquisitionFrameRate =
          pGcNode ? arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(pGcNode), &error) : FALSE;
      if (global.isImplementedAcquisitionFrameRate) {
    ...
    arv_camera_get_exposure_time_bounds(global.pCamera, &global.configMin.ExposureTimeAbs,
    ...
    if (global.isImplementedGain) arv_camera_set_gain(global.pCamera, global.config.Gain);
    ...
    if (global.isImplementedAcquisitionFrameRate)
      arv_device_set_float_feature_value(global.pDevice, global.keyAcquisitionFrameRate,
    ...
    global.pCameraInfoManager = new camera_info_manager::CameraInfoManager(
        ros::NodeHandle(ros::this_node::getName()), arv_device_get_string_feature_value(global.pDevice, "DeviceID"));
    ...
    global.config.ExposureTimeAbs =
        global.isImplementedExposureTimeAbs ? arv_device_get_float_feature_value(global.pDevice, "ExposureTimeAbs") : 0;
    global.config.Gain = global.isImplementedGain ? arv_camera_get_gain(global.pCamera) : 0.0;
    global.pszPixelformat =
        g_string_ascii_down(g_string_new(arv_device_get_string_feature_value(global.pDevice, "PixelFormat")))->str;
    global.nBytesPixel =
        ARV_PIXEL_FORMAT_BYTE_PER_PIXEL(arv_device_get_integer_feature_value(global.pDevice, "PixelFormat"));
    global.config.FocusPos =
        global.isImplementedFocusPos ? arv_device_get_integer_feature_value(global.pDevice, "FocusPos") : 0;
    ...
    ROS_INFO("    Acquisition Mode     = %s",
             global.isImplementedAcquisitionMode
                 ? arv_device_get_string_feature_value(global.pDevice, "AcquisitionMode")
                 : "(not implemented in camera)");
    ROS_INFO("    Trigger Mode         = %s", global.isImplementedTriggerMode
                                                  ? arv_device_get_string_feature_value(global.pDevice, "TriggerMode")
                                                  : "(not implemented in camera)");
    ROS_INFO("    Trigger Source       = %s", global.isImplementedTriggerSource
                                                  ? arv_device_get_string_feature_value(global.pDevice, "TriggerSource")
                                                  : "(not implemented in camera)");
    ROS_INFO("    Can set FrameRate:     %s", global.isImplementedAcquisitionFrameRate ? "True" : "False");
    if (global.isImplementedAcquisitionFrameRate) {
      global.config.AcquisitionFrameRate =
          arv_device_get_float_feature_value(global.pDevice, global.keyAcquisitionFrameRate);
    ...
    if (global.isImplementedExposureTimeAbs) {
      ...
      ROS_INFO("    Exposure             = %g us in range [%g,%g]", global.config.ExposureTimeAbs,
    ...
    if (global.isImplementedGain) {
      ...
      ROS_INFO("    Gain                 = %f %% in range [%f,%f]", global.config.Gain, global.configMin.Gain,
